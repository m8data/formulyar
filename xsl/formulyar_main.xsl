	...
	<xsl:template match="port|terminal" mode="start">
									...
									<tr>
															...
															<span style="font-size: .8em">
																...
																<xsl:choose>
																	<xsl:when test="$modifier='n'">
																		<a href="{m8:root( 'n', $fact )}" title="использовать в модификаторе" style="color: #a85">
																			<i>м</i>
																		</a>
																	</xsl:when>
																	<xsl:otherwise>
										...
										<td width="30%" align="right" valign="top">
											...
											<xsl:variable name="siblings">
												<xsl:for-each select="m8:index( $director )/object/*">
												...
												<tr>
													<td align="right">
														<xsl:if test="exsl:node-set($siblings)/*[name()=$fact]/preceding-sibling::*">
															...
															<a href="{m8:root( $predName, $modifier )}" style="{m8:color( $predName )}" title="{m8:holder( $predName )}">
													...
													<td>
														<xsl:if test="exsl:node-set($siblings)/*[name()=$fact]/following-sibling::*">
															...
															<a href="{m8:root( $nextName, $modifier )}" style="{m8:color( $nextName )}" title="{m8:holder( $nextName )}">
		...
		<xsl:if test="$fact != 'n'">
			...
			<div style="color: #777; padding: 2em;">
									...
									<xsl:choose>
										<xsl:when test="m8:index( name() )/subject">
											<a href="{m8:root( name(), $modifier )}" style="{ m8:color( name() )};" title="{ m8:holder( name() ) }">
												...
												<!--<xsl:apply-templates select="m8:port( name() )/r/*" mode="simpleName"/>-->
												<xsl:text> :: </xsl:text>
												<xsl:if test="m8:port( name(), $fact )/d[not( r )]"> [<xsl:value-of select="m8:title( name(), 'd', $fact )"/><xsl:if test="m8:port( name(), $fact )/i[not( r )]">-<xsl:value-of select="m8:title( name(), 'i', $fact )"/></xsl:if>] </xsl:if>
												<!--<xsl:apply-templates select="." mode="simpleName"/>-->
				...
				<xsl:if test="m8:role1( $fact )/*[name()!='n']">
					<!-- and $modifier = 'n'-->
					<div style="display: flex; margin: 2em">
						<div style="text-align: left; margin: 0 auto;">
							<div style="padding: .4em; text-align: center">
		...
		<xsl:if test="$fact != 'n' or $modifier != 'n' ">
					...
					<tr>
						...
						<xsl:if test="$modifier != 'n' ">
							<xsl:if test="$fact != 'n' ">
								<td valign="middle">
									<xsl:text>&#160;</xsl:text>
									<a href="{m8:root( $fact, m8:director( $modifier ) )}" title="to new Quest - { $newQuestName }">
							...
							<td valign="middle">
								<xsl:text>&#160;</xsl:text>
								<a href="{m8:root( $fact )}" title="удаление">
								...
								<xsl:choose>
												...
												<form action="{m8:root( $fact )}" id="editParamOfPort" style="margin: .5em">
													...
													<select name="r" onchange="this.form.submit()">
														...
														<xsl:for-each select="m8:index( $director )/object/*[name()!=$fact]">
															<!--and name()!='n']" xml:lang="n фильтровать, т.к. он почему-то есть в индексе самого n">-->
															<xsl:sort select="m8:title( name() )"/>
															<option value="{name()}">
									...
									<xsl:otherwise>
										<td colspan="2">Владелец - <span style="{m8:fact_color( $holder )}">
												...
												<!--<xsl:value-of select="m8:path( $fact, 'n', 'port')/@user"/>-->
												<!--<xsl:message> !!!! <xsl:value-of select="m8:holder( $fact )"/> ! <func:result select="m8:port( $fact )/@user )"/> !!!!</xsl:message>
											<xsl:value-of select="m8:holder( $fact )"/>
											<xsl:value-of select="m8:port( $fact )/@user"/>-->
											</span>
										</td>
									</xsl:otherwise>
								</xsl:choose>
								...
								<td valign="middle">
									<form action="{m8:root( $fact )}/" id="editParamOfPort" style="margin: .5em">
										...
										<select name="modifier" onchange="this.form.submit()" multiple="multiple" size="12">
											<xsl:variable name="count" select="count( m8:index( $modifier )/object/*[name()!=$fact] )"/>
					...
					<xsl:if test="( m8:holder( $fact )=$user or m8:holder( $modifier )=$user ) and ( $chiefName != 'n' or $modifier = 'n')  or not( $types ) "><!-- -->
						...
						<div style="padding: 1em; ">
							...
							<!--<div style="padding-bottom: .2em">
								<a href="{m8:root( $fact, $modifier )}&amp;r=">связь</a>
							</div>-->
							<!--<xsl:if test="$modifier != 'n' ">
								<div style="padding-bottom: .5em">
							...
							<xsl:if test="not( $startPort/n )">
								<div style="padding-bottom: .5em">
									<a href="{m8:root( $fact, $modifier )}&amp;n=">примечание</a>
								</div>
							</xsl:if>
							<xsl:message>
							START PORT:
